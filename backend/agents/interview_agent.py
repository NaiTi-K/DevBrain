...
async def interview_agent_node(state: DevBrainState) -> DevBrainState:
    ...
    # Get the last question asked by the agent for context
    last_question = ""
    ...
    if phase == PHASE_RESUME:
        ...
        if turn_count == 0:
            ...
            res = await llm.structured_call(prompt, "You are a lenient Interviewer.")
            scores[f"Resume Q{turn_count}"] = res.get("score", 7)
            agent_output, structured = await _generate_scorecard(scores, history, res.get("feedback", "Good."), structured)
        ...
        if turn_count == 0:
            ...
            res = await llm.structured_call(prompt, "You are an adaptive DSA Interviewer.")
            scores[f"DSA Q{turn_count}"] = res.get("score", 7)
            agent_output, structured = await _generate_scorecard(scores, history, res.get("feedback", "Good."), structured)
            next_phase = PHASE_SCORE
